...
class Gait():
    def __init__(self, frequency_hz, duty, swing_height=HEIGHT_SWING, phase_offset=None):
        self.gait_duty = duty
        self.gait_hz = frequency_hz
        self.swing_height = float(swing_height)
        if phase_offset is None:
            phase_offset = PHASE_OFFSET
        self.phase_offset = np.asarray(phase_offset, dtype=float).reshape(4)
    ...
    def compute_touchdown_world_for_traj_purpose_only(
        ...
        pos_norminal_term = [hip_pos_world[0], hip_pos_world[1], FOOT_TOUCHDOWN_CLEARANCE]
    ...
    def compute_swing_traj_and_touchdown(self, go2:PinGo2Model, leg:str):
        ...
        pos_touchdown_world[2] = (
            self._terrain_height(go2, pos_touchdown_world[0], pos_touchdown_world[1])
            + FOOT_TOUCHDOWN_CLEARANCE
        )
