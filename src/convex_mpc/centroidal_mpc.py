...
OPTS = {
    ...
    "osqp": dict(OSQP_PROFILE_OPTIONS["balanced"]),
}

SOLVER_NAME: str = "osqp"


def set_osqp_profile(profile: str):
    profile = str(profile).strip().lower()
    if profile not in OSQP_PROFILE_OPTIONS:
        raise ValueError(
            f"unknown OSQP profile {profile!r}; choose one of {sorted(OSQP_PROFILE_OPTIONS)}"
        )
    OPTS["osqp"] = dict(OSQP_PROFILE_OPTIONS[profile])
...
class CentroidalMPC:
    ...
    def solve_QP(self, go2:PinGo2Model, traj: ComTraj, verbose: bool = False):
        ...
        status = str(stats.get("return_status", "")).lower()
        if "infeasible" in status or "error" in status:
            raise RuntimeError(f"QP solver failed with status: {stats.get('return_status')}")
    ...
    def _compute_bounds(self, traj: ComTraj):
        ...
        # 5) Stance legs -> fz >= fz_min. FZ_MAX can be set by experiments,
        # but defaults to infinity because the simplified VBot model still
        # needs further force-limit tuning before stairs.
        ...
        ubx_np[stance_idx, 0] = np.minimum(ubx_np[stance_idx, 0], FZ_MAX)
