...
class ComTraj:
    ...
    def generate_traj(self,
                        ...
                        z_traj_world_fn=None,
                        pitch_traj_world_fn=None):

        if terrain_height_fn is None:
            terrain_height_fn = getattr(go2, "terrain_height_fn", None)
        if terrain_height_fn is not None:
            go2.terrain_height_fn = terrain_height_fn
            self.dummy_go2.terrain_height_fn = terrain_height_fn
        ...
        if not hasattr(self, "_last_generate_time") or time_now < self._last_generate_time:
            self._last_generate_time = time_now
        dt_since_last = float(np.clip(time_now - self._last_generate_time, 0.0, gait.gait_period))
        self.pos_des_world[0:2] += vel_desired_world[0:2] * dt_since_last
        self._last_generate_time = time_now
        ...
        if z_traj_world_fn is not None:
            z_ref = np.asarray(
                z_traj_world_fn(self.pos_traj_world[0, :], self.pos_traj_world[1, :]),
                dtype=float,
            ).reshape(-1)
            if z_ref.size != N:
                raise ValueError(f"z_traj_world_fn returned {z_ref.size} values, expected {N}")
            self.pos_traj_world[2, :] = z_ref


        # Linear velocity in world: constant over horizon
        self.vel_traj_world[:, :] = vel_desired_world.reshape(3, 1)
        if z_traj_world_fn is not None and N > 0:
            z_prev = float(self.pos_des_world[2])
            z_values = np.concatenate(([z_prev], self.pos_traj_world[2, :]))
            self.vel_traj_world[2, :] = np.diff(z_values) / float(time_step)

        # RPY in world:
        # Keep roll, pitch constant; integrate yaw with desired yaw rate
        self.rpy_traj_world[0, :] = 0.0
        self.rpy_traj_world[1, :] = float(pitch_des_body)
        if pitch_traj_world_fn is not None:
            pitch_ref = np.asarray(
                pitch_traj_world_fn(self.pos_traj_world[0, :], self.pos_traj_world[1, :]),
                dtype=float,
            ).reshape(-1)
            if pitch_ref.size != N:
                raise ValueError(f"pitch_traj_world_fn returned {pitch_ref.size} values, expected {N}")
        ...
        if pitch_traj_world_fn is not None and N > 0:
            pitch_prev = float(self.initial_x_vec[4])
            pitch_values = np.concatenate(([pitch_prev], self.rpy_traj_world[1, :]))
            self.omega_traj_world[1, :] = np.diff(pitch_values) / float(time_step)
        ...
        for i in range(N):
            ...
            if current_mask[0] != mask_previous[0] and current_mask[0] == 0:
                # Takes off
                pos_fl_next_td_world = gait.compute_touchdown_world_for_traj_purpose_only(
                    self.dummy_go2,
                    "FL",
                    terrain_height_fn,
                ) # This returns the next touchdown position in world coordinate
            ...
            if current_mask[1] != mask_previous[1] and current_mask[1] == 0:
                # Takes off
                pos_fr_next_td_world = gait.compute_touchdown_world_for_traj_purpose_only(
                    self.dummy_go2,
                    "FR",
                    terrain_height_fn,
                ) # This returns the next touchdown position in world coordinate
            ...
            if current_mask[2] != mask_previous[2] and current_mask[2] == 0:
                # Takes off
                pos_rl_next_td_world = gait.compute_touchdown_world_for_traj_purpose_only(
                    self.dummy_go2,
                    "RL",
                    terrain_height_fn,
                ) # This returns the next touchdown position in world coordinate
            ...
            if current_mask[3] != mask_previous[3] and current_mask[3] == 0:
                # Takes off
                pos_rr_next_td_world = gait.compute_touchdown_world_for_traj_purpose_only(
                    self.dummy_go2,
                    "RR",
                    terrain_height_fn,
                ) # This returns the next touchdown position in world coordinate
