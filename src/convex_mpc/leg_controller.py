...
class LegController():
    ...
    def compute_leg_torque(
        ...
        if current_mask[leg_idx] == 0:  # Swing phase
            ...
            # Map to joint torques + add the leg bias term. VBot and Go2 can
            # expose different joint orders, so prefer name-derived indices.
            bias = C @ go2.current_config.get_dq() + g
            if hasattr(go2, "get_leg_joint_vcols"):
                bias_leg = bias[go2.get_leg_joint_vcols(leg)]
            else:
                bias_leg = bias[joint_slice]
            tau_cmd = J_foot_world.T @ force + bias_leg
