"""EX34: real VBot MPC harness with a small state estimator.

This script is the first real-robot bridge toward the simulation MPC control
tick. Unlike EX33B, it updates the Pinocchio floating-base state from IMU and
stance-foot kinematics before running ``ComTraj + CentroidalMPC +
LegController``. The default test is deliberately all-stance and supported:
it validates state estimation, MPC force solving, joint ordering, and 100 Hz
lowcmd timing before any swing-leg gait is enabled. During the MPC control
phase it uses a Unitree-style mixed lowcmd: q/dq/kp/kd provide joint
impedance, and MPC supplies the tau feed-forward term.
"""
...
class ForwardTouchdownGait:
    """Simple velocity-based touchdown override for cautious real walking.

    The original gait's Raibert-style correction can keep feet close to the
    nominal stand pose during very slow mixed-control bring-up. This wrapper
    makes the commanded forward step explicit while leaving the contact table
    and swing trajectory shape unchanged.
    """
    ...
    def compute_touchdown_world_for_traj_purpose_only(self, go2, leg: str, terrain_height_fn=None):
        return self.gait.compute_touchdown_world_for_traj_purpose_only(
            go2,
            leg,
            terrain_height_fn,
        )
    ...
    def make_swing_trajectory(self, p0, pf, t_swing, h_sw=0.0):
        return self.gait.make_swing_trajectory(p0, pf, t_swing, h_sw)
    ...
    def compute_swing_traj_and_touchdown(self, go2, leg: str):
        ...
        base_pos = np.asarray(go2.current_config.base_pos, dtype=float).reshape(3)
        hip_offset = np.asarray(go2.get_hip_offset(leg), dtype=float).reshape(3)
        hip_world = np.array([base_pos[0], base_pos[1], 0.0]) + go2.R_z @ hip_offset

        step_x = float(go2.x_vel_des_world) * float(self.swing_time) * self.step_x_scale
        step_y = float(go2.y_vel_des_world) * float(self.swing_time) * self.step_x_scale
        touchdown = np.array([hip_world[0] + step_x, hip_world[1] + step_y, FOOT_TOUCHDOWN_CLEARANCE], dtype=float)
        height_fn = getattr(go2, "terrain_height_fn", None)
        if height_fn is not None:
            touchdown[2] = float(height_fn(float(touchdown[0]), float(touchdown[1]))) + FOOT_TOUCHDOWN_CLEARANCE
...
class PeriodicLowcmdPublisher:
    def __init__(self, pub, crc, order, hz: float, stop, latest=None, feedback_timeout_s=None):
        ...
        self._thread = threading.Thread(target=self._run, name="ex34-lowcmd-publisher", daemon=True)
...
class AsyncMPCWorker:
    """Solve MPC off the real-time leg-control path.

    The worker owns a separate robot model because the estimator and Pinocchio
    model update are mutable. The main loop submits feedback snapshots and uses
    the most recently completed MPC force while this thread works on the next
    QP.
    """
    ...
    def __init__(
        ...
        self.args = args
        self.bridge = bridge
        self.dds_model_offset = dds_model_offset
        self.vbot = PinVBotModel()
        self.estimator = RealStateEstimator(args)
        self.gait = build_gait(args)
        self.traj = ComTraj(self.vbot)
        self.CentroidalMPC = CentroidalMPC
        self.mpc = None
        self.mpc_dt = self.gait.gait_period / args.horizon_segments
        self.period = 1.0 / args.mpc_hz
        self.solve_time = 0.0
        self.latest_force = np.zeros(12, dtype=float)
        self.count = 0
        self.time_sum = 0.0
        self.time_max = 0.0
        self.solve_fail_count = 0
        self.solve_fail_limit = 3
        self.last_solve_error = ""
        self.error = None
        self._pending = None
        self._stop = False
        self._cond = threading.Condition()
        self._thread = threading.Thread(target=self._run, name="ex34-async-mpc", daemon=True)
    ...
    def submit(self, cache, base, contact_mask, elapsed_s, now_s):
        import numpy as np

        cache_copy = {joint: dict(values) for joint, values in cache.items()}
        base_copy = dict(base)
        base_copy["gyro"] = tuple(base["gyro"])
        with self._cond:
            self._pending = {
                "cache": cache_copy,
                "base": base_copy,
                "contact_mask": np.asarray(contact_mask, dtype=int).reshape(4).copy(),
    ...
    def get_force(self):
        ...
        with self._cond:
            return np.asarray(self.latest_force, dtype=float).copy()
    ...
    def snapshot_stats(self):
        with self._cond:
            return {
                ...
                "solve_fail_count": self.solve_fail_count,
                "last_solve_error": self.last_solve_error,
                "error": self.error,
            }
    ...
    def _run(self):
        ...
        while True:
            with self._cond:
                ...
                now = time.monotonic()
                if now < next_start:
                    self._cond.wait(timeout=next_start - now)
                    continue
                work = self._pending
                self._pending = None
            ...
            try:
                ...
                generate_traj_compat(
                    ...
                    self.args.yaw_rate,
                    self.mpc_dt,
                    self.args.base_pitch,
                )
                if self.mpc is None:
                    self.mpc = self.CentroidalMPC(self.vbot, self.traj)
                try:
                    sol = self.mpc.solve_QP(self.vbot, self.traj, self.args.verbose_mpc)
                except Exception as exc:
                    with self._cond:
                        self.solve_fail_count += 1
                        self.last_solve_error = f"{type(exc).__name__}: {exc}"
                        if self.solve_fail_count >= self.solve_fail_limit:
                            self.error = (
                                f"mpc_solve_failed_{self.solve_fail_count}x: "
                                f"{self.last_solve_error}"
                            )
                            return
                    next_start = advance_periodic_deadline(next_start, self.period, time.monotonic())
                    continue
                w_opt = sol["x"].full().flatten()
                u_opt = w_opt[12 * self.traj.N :].reshape((12, self.traj.N), order="F")
                force = filter_mpc_force_now(self.args, u_opt[:, 0])
                elapsed = time.monotonic() - solve_t0
                with self._cond:
                    self.latest_force = np.asarray(force, dtype=float).copy()
                    self.solve_fail_count = 0
                    self.last_solve_error = ""
                    self.solve_time = float(getattr(self.mpc, "solve_time", 1000.0 * elapsed))
                    self.count += 1
                    self.time_sum += elapsed
                    self.time_max = max(self.time_max, elapsed)
                next_start = advance_periodic_deadline(next_start, self.period, time.monotonic())
            except Exception as exc:
                with self._cond:
                    self.error = f"{type(exc).__name__}: {exc}"
...
def build_arg_parser(profile_defaults=None):
    parser = argparse.ArgumentParser(description="EX34 real VBot MPC with state estimator")
    parser.add_argument("--test-profile", choices=("custom", "trot-final", "all-stance"), default="custom")
    parser.add_argument("--config", default=None, help="YAML file with EX34 argument defaults")
    ...
    parser.add_argument("--async-mpc", action="store_true", default=False)
    parser.add_argument("--sync-mpc", action="store_false", dest="async_mpc")
    parser.add_argument("--osqp-profile", choices=("balanced", "fast"), default="balanced")
    ...
    parser.add_argument("--swing-qd-target-limit", type=float, default=-1.0, help="clip swing qd target; negative leaves it unclipped")
    parser.add_argument("--swing-ik-mode", choices=("iterative", "analytic"), default="iterative")
    parser.add_argument("--forward-touchdown", action="store_true")
    parser.add_argument("--no-forward-touchdown", action="store_false", dest="forward_touchdown")
    parser.add_argument("--forward-touchdown-scale", type=float, default=1.0)
    ...
    parser.add_argument("--swing-tau-ff-scale", type=float, default=0.0)
    ...
    parser.add_argument("--log-prefix", default="ex34_real_mpc_state_estimator")
...
def load_ex34_config_defaults(path):
    try:
        import yaml
    except Exception as exc:
...
def defaults_from_argv(argv=None):
    ...
    config_defaults = load_ex34_config_defaults(args.config) if args.config else {}
...
def validate_args(args):
    if not args.robot_standing_supported:
        raise RuntimeError("EX34 requires --robot-standing-supported")
    if not args.i_accept_risk:
        raise RuntimeError("EX34 sends real motor commands; pass --i-accept-risk")
    ...
    if args.forward_touchdown_scale < 0.0:
        raise RuntimeError("--forward-touchdown-scale must be non-negative")
...
def swing_target_from_leg_output(vbot, leg, out, q_model, args):
    ...
    if args.swing_ik_mode == "analytic" and hasattr(vbot, "calc_leg_ik_body"):
        q_des = vbot.calc_leg_ik_body(leg, foot_des_body)
    else:
...
def tau_scale_vector_for_contact(args, bridge, contact_mask):
    ...
    for i, joint in enumerate(bridge.order):
        leg = joint.split("_", 1)[0]
        in_swing = args.gait != "all-stance" and int(contact_mask[LEG_INDEX[leg]]) == 0
        scale[i] = float(args.swing_tau_ff_scale) if in_swing else float(args.tau_ff_scale)
    return scale
...
def compute_tau_from_force(vbot, gait, leg_controller, mpc_force_now, gait_time_s, args):
    import numpy as np
    from convex_mpc.leg_controller import KP_SWING, KD_SWING, LegOutput

    tau_model = np.zeros(12, dtype=float)
    contact_mask = gait.compute_current_mask(gait_time_s).reshape(4)
    leg_outputs = {}
    need_bias = bool(args.stance_bias_comp)
    need_swing = args.gait != "all-stance" and np.any(contact_mask == 0) and args.allow_swing_control
    bias = None
    M = None
    if need_bias or need_swing:
        g, C, M = vbot.compute_dynamcis_terms()
        bias = np.asarray(C @ vbot.current_config.get_dq() + g, dtype=float).reshape(-1)
    for leg, leg_slice in LEG_SLICE.items():
        leg_idx = LEG_INDEX[leg]
        in_swing = int(contact_mask[LEG_INDEX[leg]]) == 0 and args.gait != "all-stance"
        if in_swing and not args.allow_swing_control:
            if need_bias and bias is not None:
                tau_model[leg_slice] = bias[vbot.get_leg_joint_vcols(leg)]
            leg_controller.last_mask[leg_idx] = contact_mask[leg_idx]
            continue

        J_foot_world = vbot.compute_3x3_foot_Jacobian_world(leg)
        foot_pos_des, foot_vel_des = vbot.get_single_foot_state_in_world(leg)
        foot_pos_now, foot_vel_now = foot_pos_des, foot_vel_des

        if in_swing:
            if leg_controller.last_mask[leg_idx] != contact_mask[leg_idx]:
                setattr(leg_controller, f"{leg}_takeoff_time", gait_time_s)
                traj, td_pos = gait.compute_swing_traj_and_touchdown(vbot, leg)
                setattr(leg_controller, f"{leg}_traj", traj)
                setattr(leg_controller, f"{leg}_td_pos", td_pos)
            if not hasattr(leg_controller, f"{leg}_traj"):
                setattr(leg_controller, f"{leg}_takeoff_time", gait_time_s)
                traj, td_pos = gait.compute_swing_traj_and_touchdown(vbot, leg)
                setattr(leg_controller, f"{leg}_traj", traj)
                setattr(leg_controller, f"{leg}_td_pos", td_pos)

            takeoff_time = getattr(leg_controller, f"{leg}_takeoff_time")
            traj = getattr(leg_controller, f"{leg}_traj")
            foot_pos_des, foot_vel_des, foot_acc_des = traj(gait_time_s - takeoff_time)
            foot_pos_now, foot_vel_now = vbot.get_single_foot_state_in_world(leg)
            pos_error = foot_pos_des - foot_pos_now
            vel_error = foot_vel_des - foot_vel_now

            J_full_foot_world = vbot.compute_full_foot_Jacobian_world(leg)
            operational_mass_inv = J_full_foot_world @ np.linalg.solve(M, J_full_foot_world.T)
            Lambda = np.linalg.inv(operational_mass_inv)
            Jdot_dq = vbot.compute_Jdot_dq_world(leg)
            f_ff = Lambda @ (foot_acc_des - Jdot_dq)
            force = KP_SWING @ pos_error + KD_SWING @ vel_error + f_ff
            tau_leg = J_foot_world.T @ force + bias[vbot.get_leg_joint_vcols(leg)]
        else:
            tau_leg = J_foot_world.T @ -np.asarray(mpc_force_now[leg_slice], dtype=float).reshape(3)
            if need_bias and bias is not None:
                tau_leg = tau_leg + bias[vbot.get_leg_joint_vcols(leg)]

        tau_model[leg_slice] = tau_leg
        # Stance legs use MPC contact force. Swing legs are kept here so their
        # foot trajectory can be converted to q/qd targets; swing tau is not
        # sent to the low-level command in this real-robot bring-up path.
        leg_outputs[leg] = LegOutput(
            tau=np.asarray(tau_leg, dtype=float).reshape(3),
            pos_des=np.asarray(foot_pos_des, dtype=float).reshape(3),
            pos_now=np.asarray(foot_pos_now, dtype=float).reshape(3),
            vel_des=np.asarray(foot_vel_des, dtype=float).reshape(3),
            vel_now=np.asarray(foot_vel_now, dtype=float).reshape(3),
        )
        leg_controller.last_mask[leg_idx] = contact_mask[leg_idx]
...
def main() -> int:
    ...
    centroidal_mpc_module.set_osqp_profile(args.osqp_profile)
    ...
    print("EX34 DDS real MPC with state estimator")
    leg_control_hz = args.cmd_hz if args.leg_control_hz is None else args.leg_control_hz
    print(
        f"network={args.network} gait={args.gait} cmd_hz={args.cmd_hz:.1f} "
        f"leg_hz={leg_control_hz:.1f} mpc_hz={args.mpc_hz:.1f} "
        f"mpc_mode={'async' if args.async_mpc else 'sync'} "
        f"osqp_profile={args.osqp_profile}"
    )
    ...
    print(
        ...
        f"stance_q_delta_scale={args.stance_mpc_q_delta_scale:.2f} "
        f"swing_q_delta_limit={args.swing_q_target_delta_limit:.3f} "
        f"forward_touchdown={bool(args.forward_touchdown)}:{args.forward_touchdown_scale:.2f}"
    )
    print(
    ...
    if args.dds_model_space:
        print("mapping: DDS q/dq are gateway model-space; EX34 converts MPC tau to motor-side sign/scale")
    else:
        print("mapping: raw motor-space debug path; EX34 applies affine q/tau conversion before LowCmd")
    if args.gait != "all-stance":
    ...
    try:
        ...
        if args.dds_model_space:
            ...
            raise RuntimeError("EX34 currently expects --dds-model-space gateway mapping")
        ...
        if args.async_mpc:
            ...
            async_mpc_worker = mpc
            async_mpc_worker.start()
            async_mpc_worker.submit(cache, base, contact_mask, 0.0, time.monotonic())
        else:
        ...
        while time.monotonic() < deadline and not stop["requested"]:
            ...
            if async_mpc_worker is not None:
                mpc_error = async_mpc_worker.get_error()
                if mpc_error:
                    exit_reason = f"async_mpc:{mpc_error}"
                    raise RuntimeError(f"async MPC failed: {mpc_error}")
            ...
            if now >= next_mpc:
                if async_mpc_worker is not None:
                    async_mpc_worker.submit(cache, base, contact_mask, now - start_t, now)
                    mpc_force_now = async_mpc_worker.get_force()
                else:
        ...
        if async_mpc_worker is not None:
            async_mpc_worker.stop_and_join()
            mpc_stats = async_mpc_worker.snapshot_stats()
            mpc_update_count = mpc_stats["count"]
            mpc_time_sum = mpc_stats["time_sum"]
            mpc_time_max = mpc_stats["time_max"]
            mpc_solve_fail_count = mpc_stats.get("solve_fail_count", 0)
            mpc_last_solve_error = mpc_stats.get("last_solve_error", "")
            if mpc_stats["error"] and exit_reason in ("not_started", "completed_duration"):
                exit_reason = f"async_mpc:{mpc_stats['error']}"
        ...
        write_summary(
            ...
            extra={
                ...
                "mpc_solve_fail_count": mpc_solve_fail_count if async_mpc_worker is not None else 0,
                "mpc_last_solve_error": mpc_last_solve_error if async_mpc_worker is not None else "",
